...
class TimelineService @Inject()(val postDao: PostDao, val userDao: UserDao):
  ...
  def getLatestPosts: Either[Unit, (List[(PostRecord, User)], Boolean)] =
  ...
  def getLatestPostsNewerThan(postId: Int): Either[Unit, (List[(PostRecord, User)], Boolean)] =
  ...
  def getLatestPostsOlderThan(postId: Int): Either[Unit, (List[(PostRecord, User)], Boolean)] =
    combineWithUser(postDao.selectLatestBefore(postId, POST_COUNT_IN_TIMELINE + 1))
      .map(list => (list.slice(0, POST_COUNT_IN_TIMELINE), list.size == POST_COUNT_IN_TIMELINE + 1))

  private def combineWithUser(postRecords: List[PostRecord]): Either[Unit, List[(PostRecord, User)]] = {
